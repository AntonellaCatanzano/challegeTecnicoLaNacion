...
namespace ChallegeContactRecords.WebApi.Repositories
{
    ...
    {
        private readonly ContactRecordsDBContext _dbContext;
        public ContactRecordsRepository(ContactRecordsDBContext dbContext)
        {
        ...
        {
            // var contact = GetContactRecordByID(contactRecord.ID);

            var contact = await _dbContext.ContactsRecord.FirstOrDefaultAsync(e => e.ID == contactRecord.ID);

            //if (contact != null)
            //{
            //    _dbContext.ContactsRecord.Update(contactRecord);
            //    await _dbContext.SaveChangesAsync();
            //}
